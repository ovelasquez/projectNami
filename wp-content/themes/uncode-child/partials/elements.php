        ...
        foreach ($layout as $key => $value) {
            switch ($key) {
                ...
                case 'link':
                    ...
                    if ($single_text === 'overlay' && $single_elements_click !== 'yes')
                        $inner_entry .= '<p class="t-entry-readmore"><span class="btn' . $btn_shape . '">' . esc_html__('Read More', 'uncode') . ' </span></p>';
                    else
                        $inner_entry .= '<p class="t-entry-readmore"><a href="' . $create_link . '" class="btn' . $btn_shape . '">' . esc_html__('Read More', 'uncode') . ' </a></p>';
                    break;
                ...
                case 'id-phy':
                    ...
                    if (isset($get_cf_value) && $get_cf_value !== '')
                        $inner_entry.= ''
                                . '<a class="t-entry-cf-' . $key . ' btn btn-color-164525" href="' . $title_link . '" data-toggle="tooltip" data-placement="top" data-original-title="Consulta" data-title="" title="">Solicitar Consulta</a>';
            ...
            if (array_key_exists('media', $layout)) :
                ...
                if ($style_preset === 'metro'):
                    ...
                    if ($media_type === 'image') :
                        ...
                        //ICONO de telesalvavidas 
                        $catTs = false;
                        $categories = get_the_category();
                        if ($categories) {
                            foreach ($categories as $category) {
                                if ($category->term_id == 101):
                                    $postAct = get_post();
                                    $media = get_attached_media('image', $postAct->ID);
                                    if (array_key_exists($block_data["media_id"], $media)):
                                        $catTs = true;
                                    endif;
                                    break;
                                endif;
                        ...
                        $icoTs = '';
                        if ($catTs || (is_array($block_data['single_categories_id']) && in_array(101, $block_data['single_categories_id']))):
                            $icoTs = '<span class="icon-ts-list"><img src="' . get_stylesheet_directory_uri() . '/img/telesalvavidas-logo-1.png" /> </span>';

                        endif;                      
                                                
                        $output .= $icoTs.''.apply_filters('post_thumbnail_html', '<img' . ( $adaptive_async_class !== '' ? ' class="' . trim($adaptive_async_class) . '"' : '' ) . ' src="' . $item_media . '" width="' . $image_orig_w . '" height="' . $image_orig_h . '" alt="' . $media_alt . '"' . ( $adaptive_async_data !== '' ? $adaptive_async_data : '' ) . ' />', $product_id);
            ...
            if (class_exists('WooCommerce') && $is_product) {
                ...
                if (!empty($product)) {
                    if (get_option('woocommerce_hide_out_of_stock_items') == 'yes' && !$product->is_in_stock())
                        return;
                    $product_add_to_cart = sprintf('<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" class="%s %s product_type_%s ">%s</a>', esc_url($product->add_to_cart_url()), esc_attr($block_data['id']), esc_attr($product->get_sku()), $product->is_purchasable() && $product->is_in_stock() ? 'add_to_cart_button' : '', $product->supports('ajax_add_to_cart') && get_option('woocommerce_enable_ajax_add_to_cart') == 'yes' ? 'ajax_add_to_cart' : '', esc_attr($product->get_type()), esc_html($product->add_to_cart_text())
                    );
...
if (!function_exists('uncode_breadcrumbs')) {
    ...
    function uncode_breadcrumbs($navigation_index = '') {
        ...
        if (is_home() || is_front_page()) {
            ...
            if ($show_on_home == 1)
                $html = '<ol vocab="http://schema.org/" typeof="BreadcrumbList"><li property="itemListElement" typeof="ListItem"><a href="' . $home_link . '">' . $text['home'] . '</a></li></ol>';
        } else {
            ...
            if (is_category()) {
                ...
                if (get_post_type() != 'post') {
                    ...
                    if ($navigation_index !== '') {
                        ...
                        $parent_link = $home_link . ltrim($slug['slug'], '/');
