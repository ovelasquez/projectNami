...
while (have_posts()):
	...
	if ($navigation_option !== 'off') {
        ...
        /** Load info **/        
        $id_phy = get_post_meta(get_the_ID(), 'id-phy', true);
        $opts = array('http'=>array('header' => "User-Agent:MyAgent/1.0\r\n"));
        $context = stream_context_create($opts);
        $json_url = "https://www.medeconsult.com/consultas-medicas/web/app_dev.php/physicians/info/".$id_phy."?r=".rand();
        $json = file_get_contents($json_url,false,$context);
        $data = json_decode($json, TRUE);
        
        $cont = uncode_remove_wpautop($the_content);
        if (count($data)>0):            
            $bio = str_replace("\r\n\r\n","<br/><br/>",$data[0]['biography']);            
            $cont = str_replace('[BIO]',$bio,$cont);
            $cont = str_replace('[NAME]',$data[0]['name']." ".$data[0]['last_name'],$cont);
            $cont = str_replace('[JOBTITLE]',$data[0]['jobtitle'],$cont);
            $cont = str_replace('wp-content/uploads/2017/04/medico.jpg','/consultas-medicas/web/uploads/documents/'.$data[0]['photo'].'',$cont);
            if($data[0]['volunteering_vzla']!=="1"):
                $cont = str_replace('ts-box','ts-box hidden ',$cont);
            endif;
        endif;
        /** Load info **/
	...
	echo 	'<article id="post-'. get_the_ID().'" class="'.implode(' ', get_post_class('page-body' . $bg_color)) .'">
          <div class="post-wrapper">
          	<div class="post-body especialista">' . $cont . '</div>' .
          	$navigation_content . '
          </div>
        </article>';
